                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button aria-label={`${camera.isActive ? "Nonaktifkan" : "Aktifkan"} capture ${camera.name}`} aria-pressed={camera.isActive} onClick={(event) => { event.stopPropagation(); toggleCamera(camera.id, camera.isActive); }} onKeyDown={(event) => event.stopPropagation()} disabled={updateCamera.isPending || !isAdmin} title={isAdmin ? undefined : "Mode Guest hanya dapat melihat status capture"} className={`relative h-6 w-11 rounded-full transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${camera.isActive ? "bg-lime-400" : "bg-stone-200"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${camera.isActive ? "translate-x-6" : "translate-x-1"}`} /></button>
