...
export default function CameraDetail() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-7">
      ...
      {failureExplanation ? <div role="status" className={`rounded-2xl border p-4 ${failureLabel === "Sumber HLS gagal" ? "border-orange-200 bg-orange-50" : "border-red-200 bg-red-50"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className={`text-sm font-semibold ${failureLabel === "Sumber HLS gagal" ? "text-orange-900" : "text-red-900"}`}>{failureLabel}</p><p className="mt-1 text-sm leading-6 text-stone-700">{failureExplanation}</p><p className="mt-2 text-xs text-stone-500">Periksa URL dan status worker sebelum mengubah konfigurasi kamera.</p></div>{failureLabel === "Sumber HLS gagal" ? <Button type="button" variant="outline" onClick={() => setPlayerAttempt((attempt) => attempt + 1)} className="shrink-0 rounded-xl border-orange-300 bg-white text-orange-900 hover:bg-orange-100"><RefreshCw className="mr-2 h-3.5 w-3.5" />Muat ulang live view</Button> : <Link href="/cameras" className="inline-flex h-10 shrink-0 items-center rounded-xl border border-red-300 bg-white px-3 text-sm font-semibold text-red-900 hover:bg-red-100">Tinjau registry</Link>}</div></div> : null}
