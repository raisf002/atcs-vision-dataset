...
import { Activity, ArrowLeft, Camera, Check, Database, ExternalLink, FolderKey, Save, Settings2 } from "lucide-react";
...
export default function CameraDetail() {
  ...
  const camera = cameraRegistry.find((item) => item.id === params?.cameraId);
  const [sourceUrl, setSourceUrl] = useState(camera?.sourceUrl ?? "");
  const [captureEnabled, setCaptureEnabled] = useState(camera?.active ?? false);
  const [interval, setInterval] = useState<(typeof intervals)[number]>("5");
  const { user } = useAuth();
  const updateCamera = trpc.dataset.updateCamera.useMutation();

  useEffect(() => {
    const rawDraft = camera ? localStorage.getItem(`atcs-camera-draft-${camera.id}`) : null;
    if (rawDraft) {
      try {
        ...
        setCaptureEnabled(draft.isActive ?? camera?.active ?? false);
        setInterval(draft.captureIntervalMinutes ?? "5");
        return;
      } catch {
    ...
    setCaptureEnabled(camera?.active ?? false);
    setInterval("5");
  }, [camera]);
  ...
  const saveDraft = () => {
    ...
    updateCamera.mutate({ id: camera.id, sourceUrl: sourceUrl || null, isActive: captureEnabled, captureIntervalMinutes: interval }, {
      onSuccess: () => toast.success("Konfigurasi kamera disimpan."),
