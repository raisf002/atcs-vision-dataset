...
import { Activity, Camera, CircleDot, Expand, Gauge, Map as MapIcon, Maximize2, RotateCcw, ScanLine, Target, Video } from "lucide-react";
    ...
    <div className="-m-4 min-h-[calc(100vh-4rem)] bg-[#080b0d] text-white sm:-m-6 lg:-m-8">
      ...
      <main className="grid gap-3 p-3 sm:p-4 xl:grid-cols-[minmax(0,1fr)_330px]">
        ...
        <aside className="space-y-3">
          ...
          <CountingWorkspace camera={selected ? { id: selected.id, name: selected.name } : undefined} overlayTargetId={selected ? `counting-overlay-${selected.id}` : "counting-overlay"} overlayElement={countingOverlayElement} isConsoleActive={activeTab === "console"} onOpenConsole={() => setActiveTab("console")} />
