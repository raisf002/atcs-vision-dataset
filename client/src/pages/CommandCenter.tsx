    ...
    <div className="-m-4 min-h-[calc(100vh-4rem)] bg-[#080b0d] text-white sm:-m-6 lg:-m-8">
      ...
      <main className="grid gap-3 p-3 sm:p-4 xl:grid-cols-[minmax(0,1fr)_330px]">
        <section className="min-w-0 space-y-3">
          ...
          {activeTab === "console" && selected ? <div className="rounded-xl border border-white/10 bg-[#101719] p-2"><LiveHlsPlayer sourceUrl={selected.sourceUrl} cameraName={selected.name} /><div className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-2"><button type="button" onClick={resetCamera} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-[11px] font-semibold text-stone-300 hover:bg-white/10"><RotateCcw className="h-3.5 w-3.5" />Reset kamera</button><button type="button" onClick={() => setAiActive((value) => !value)} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-[11px] font-semibold ${aiActive ? "border-lime-300/30 bg-lime-300/10 text-lime-200" : "border-white/10 text-stone-300 hover:bg-white/10"}`}><Activity className="h-3.5 w-3.5" />AI deteksi: {aiActive ? "AKTIF (preview)" : "STANDBY"}</button><button type="button" onClick={() => toast.info("Zona deteksi akan tersedia setelah koordinat area kamera ditetapkan.")} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-[11px] font-semibold text-stone-300 hover:bg-white/10"><Target className="h-3.5 w-3.5" />Set zona deteksi</button><span className="ml-auto flex items-center gap-2"><button type="button" onClick={takeSnapshot} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-[11px] font-semibold text-stone-300 hover:bg-white/10"><Camera className="h-3.5 w-3.5" />Snapshot</button><button type="button" onClick={toggleFullscreen} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-[11px] font-semibold text-stone-900 hover:bg-lime-200"><Expand className="h-3.5 w-3.5" />Fullscreen</button></span></div></div> : <div className="relative min-h-[520px] overflow-hidden rounded-xl border border-white/10 bg-[#0c1214] p-5"><div className="absolute inset-0 opacity-30" style={{ backgroundImage: "linear-gradient(rgba(111,140,130,.18) 1px, transparent 1px), linear-gradient(90deg, rgba(111,140,130,.18) 1px, transparent 1px)", backgroundSize: "48px 48px" }} /><div className="relative flex h-full min-h-[480px] flex-col justify-between"><div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-stone-400"><MapIcon className="h-4 w-4 text-lime-300" />Peta registry ATCS · overview</div><p className="mt-2 max-w-md text-sm text-stone-500">Pilih salah satu titik kamera untuk membuka konsol live view. Posisi visual ini adalah indeks registry, bukan koordinat GIS produksi.</p></div><div className="relative mx-auto h-[340px] w-full max-w-[760px]">{cameras.map((camera, index) => { const left = 8 + ((index * 37) % 84); const top = 12 + ((index * 53) % 76); const active = camera.id === selected?.id; const tone = camera.lastCaptureStatus === "failed" ? "bg-orange-300" : camera.lastCaptureStatus === "success" ? "bg-lime-300" : "bg-stone-500"; return <button key={camera.id} type="button" onClick={() => selectCamera(camera.id)} aria-label={`Pilih ${camera.name}`} className={`group absolute -translate-x-1/2 -translate-y-1/2 rounded-md border px-1.5 py-1 font-mono text-[9px] transition-all ${active ? "z-10 scale-125 border-lime-200 bg-lime-300 text-lime-950 shadow-[0_0_0_4px_rgba(190,242,100,.12)]" : "border-white/10 bg-black/50 text-stone-300 hover:scale-110 hover:border-lime-300/50"}`} style={{ left: `${left}%`, top: `${top}%` }}><span className={`mr-1 inline-block h-1.5 w-1.5 rounded-full ${tone}`} />{String(index + 1).padStart(2, "0")}<span className="pointer-events-none absolute bottom-full left-1/2 mb-1 hidden -translate-x-1/2 whitespace-nowrap rounded bg-black px-2 py-1 text-[9px] text-white group-hover:block">{camera.name}</span></button>; })}</div><div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-black/35 px-3 py-2 text-[11px] text-stone-400"><span><span className="mr-2 inline-block h-2 w-2 rounded-full bg-lime-300" />Berhasil <span className="ml-1 text-white">{summary.success}</span></span><span><span className="mr-2 inline-block h-2 w-2 rounded-full bg-orange-300" />Sumber/pipeline gagal <span className="ml-1 text-white">{summary.failed}</span></span><span><span className="mr-2 inline-block h-2 w-2 rounded-full bg-stone-500" />Menunggu/nonaktif <span className="ml-1 text-white">{summary.pending + summary.disabled}</span></span></div></div></div>}

          <div className="grid gap-3 sm:grid-cols-4"><div className="rounded-xl border border-white/10 bg-[#101719] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-stone-500">Kamera terpantau</p><p className="mt-2 text-2xl font-semibold text-white">{cameras.length}</p><p className="mt-1 text-[10px] text-stone-500">20 kota · 9 nasional</p></div><div className="rounded-xl border border-white/10 bg-[#101719] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-stone-500">Live source</p><p className="mt-2 text-2xl font-semibold text-lime-300">{summary.success}</p><p className="mt-1 text-[10px] text-stone-500">capture terakhir berhasil</p></div><div className="rounded-xl border border-white/10 bg-[#101719] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-stone-500">Snapshot</p><p className="mt-2 text-2xl font-semibold text-white">{summary.snapshots}</p><p className="mt-1 text-[10px] text-stone-500">metadata tersimpan</p></div><div className="rounded-xl border border-white/10 bg-[#101719] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-stone-500">Status source</p><p className={`mt-2 text-sm font-semibold ${sourceStatus === "verified" ? "text-lime-300" : "text-orange-300"}`}>{sourceLabel}</p><p className="mt-1 truncate text-[10px] text-stone-500">{formatTime(selected?.lastCaptureAt)}</p></div></div>
        </section>

        <aside className="space-y-3">
          <section className="rounded-xl border border-white/10 bg-[#101719] p-4"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 text-xs font-bold text-white"><Activity className="h-3.5 w-3.5 text-lime-300" />Telemetri dataset</h2><button type="button" onClick={() => camerasQuery.refetch()} className="text-[10px] text-stone-500 hover:text-white">Refresh</button></div><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-lg border border-white/10 bg-black/15 p-3"><p className="text-[10px] text-stone-500">Capture berhasil</p><p className="mt-1 text-2xl font-semibold text-lime-300">{summary.success}</p><p className="mt-1 text-[10px] text-stone-600">kamera</p></div><div className="rounded-lg border border-white/10 bg-black/15 p-3"><p className="text-[10px] text-stone-500">Sumber gagal</p><p className="mt-1 text-2xl font-semibold text-orange-300">{summary.failed}</p><p className="mt-1 text-[10px] text-stone-600">perlu review</p></div></div><div className="mt-2 rounded-lg border border-white/10 bg-black/15 p-3"><div className="flex items-center justify-between"><p className="text-[10px] text-stone-500">Total snapshot</p><span className="text-[10px] text-stone-600">metadata</span></div><p className="mt-1 text-3xl font-semibold text-white">{summary.snapshots}</p><p className="mt-1 text-[10px] text-stone-500">Akumulasi dari seluruh kamera</p></div></section>
