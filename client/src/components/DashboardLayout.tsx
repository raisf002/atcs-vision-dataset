...
const menuItems = [
  ...
  { icon: Radar, label: "Pusat Komando", path: "/command-center" },
  { icon: Camera, label: "Registri Kamera", path: "/cameras" },
  { icon: GalleryVerticalEnd, label: "Galeri Dataset", path: "/dataset" },
  { icon: Archive, label: "Ekspor", path: "/exports" },
  { icon: Settings2, label: "Pengaturan", path: "/settings" },
];
...
function DashboardLayoutContent({ children, demoMode, setSidebarWidth }: { children: React.ReactNode; demoMode: boolean; setSidebarWidth: (width: number) => void }) {
  ...
  const identityDetail = demoMode ? "Pratinjau antarmuka" : isGuest ? "Mode baca publik" : user?.email || "Administrator";
